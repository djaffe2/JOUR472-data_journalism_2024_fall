...
**A1.**
```{r}
md_senate_clean <- read_csv("data/md_senate_purpose.csv")
```
...
**A2.** The largest contributions come from media advertising and salary. 
...
```{r}
md_senate_clean <- md_senate_clean |>
...
**A4.** I used ChatGPT for this questions because I kept getting the error that purpose_clean is not found. I asked ChatGPT what my error was and used the revised code but it is still not working.
...
**A5** It is interesting to see which candidate prioritized their spending and where they invested in most. I think there could be a story if one of the candidates prioritized something more/less than the other and if it directly correlated to how their campaign turned out.
