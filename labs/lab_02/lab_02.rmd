...
md_voters_2024 |>
  filter (COUNTY_REGISTRATION_DATE >=ymd('07-21-2024')
  ) |>
  group_by(COUNTY)
...
**A4** 
I used ChatGPT for the secon part of this code but am still confused on how to complete it 
```{r}
md_voters_2024 |>
  mutate(region= case_when(
    region_1 <- c("COUNTY 1", "COUNTY 2", "COUNTY 3")
    region_2 <- c( "COUNTY 1", "COUNTY 2", "COUNTY 3")
    region_3 <- c("COUNTY 1", "COUNTY 2", "COUNTY 3")
    region_4<- c("COUNTY 1", "COUNTY 2", "COUNTY 3")
    region_5<- c("COUNTY 1", "COUNTY 2", "COUNTY 3")
  
md_voters_2024 |>
  mutate(region = case_when(
  COUNTY %in% region_1 ~"Region 1"
  COUNTY %in% region_2 ~"Region 2"
  ))
...
**A5** Did more Republican counties in Maryland have an increase in voter registration?
