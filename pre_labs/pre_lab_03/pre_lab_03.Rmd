...
**Answer**One or more parsing issues, call `problems()` on your data frame for details, e.g.:
  dat <- vroom(...)
  problems(dat)
...
**Answer** It is showing the amount of issues in the table
...
**Answer** No it shows the full amount of rows and columns
...
**Answer**: 402,345- people voted in person 
...
**Answer** What does deceased mean in this context? I would want to explore that especially since the number is so high. The legacy data, moved from county and confirmed not returned also stick out to me.
...
**Answer**
-The amount column is stored as a character, not a number. If we try to do math to it – say, calculate the average contribution size – it won’t work.
-There’s a fully duplicated row – a common problem in data sets. The 14th row is exactly the same as the seventh.
-The city field has five different forms – including misspellings – of Conowingo. If we wanted to group and count the number of contributions in Conowingo, this inconsistency would not let us do that correctly.
...
**Answer** It added an x before the header. This code 'clean_names()' helps with cleaning the data and fixing headlines/column names 
...
**Task** Run the following codeblock to attempt to add up the amount of all contributions. What does the error say when you run this code? What do you think the error means? Answer below. **Answer** An error shows up because the amount column is stored as a character when it should be a variable
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many are there? What is the donor name? Answer below. **Answer** 4
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. Add a description of what this code does to your reference notebook. **Answer** 13

distinct() gets rid of duplicates in the table
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, standardizing the field. Look at the difference in the result - what changed? **Answer** Only the first five numbers of the zip code were kept 
...
4 errors. It makes all the spellings of the town in the correct format.
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field. How many mispellings of Conowingo remain after running this code? Answer below. **Answer**1
