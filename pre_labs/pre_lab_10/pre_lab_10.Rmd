...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer** There are 10 columns in this data and the include precinct's and their names and ids. It also includes Congress, Legislatures, Councils and Schools but they are numbers
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer** The different precincts and the features of that precinct.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The output is a map with coordinates of PG county. It shows the precincts as well.
...
**Task** Run the following code. Describe the output in the space below. **Answer** the library loaded
...
**Task** Run the following code. Describe the output in the space below. **Answer** It has a new column called geometry. It also has a lot more data
...
**Task** Run the following code. Describe the output in the space below. **Answer** It filtered to only MD counties using the state FP number
...
**Task** Run the following code. Describe the output in the space below. **Answer** a map of MD counties with latitude and longtitude 
...
**Task** Run the following code. Describe the output in the space below. **Answer**It shows the PG county voter turnout for the July primary election
...
**Task** Run the following code. Describe the output in the space below. **Answer** It shows the PG county precincts with the voter turnout dataset
...
**Task** Run the following code. Describe the output in the space below. **Answer** A lot of voters are not eligible. In the middle it is a higher number
...
**Task** Run the following code. Describe the output in the space below. **Answer** The counties with higher populations are at the top. Montgomery County has the most, followed by Prince George's and Baltimore County
...
**Task** Run the following code. Describe the output in the space below. **Answer** Population from each MD county
...
**Task** Run the following code. Describe the output in the space below. **Answer**There is a new column, geometry, that contains the "MULTIPOLYGON" feature. It also shows different coordinates 
...
**Task** Run the following code. Describe the output in the space below. **Answer**The columns are cleaner and it removed any white spaces that would prevent a join
...
**Task** Run the following code. Describe the output in the space below. **Answer**It joined both data frames so we can have the voters per 10k residents in each county
...
**Task** Run the following code. Describe the output in the space below. **Answer**It fixed the St. Mary's and Baltimore COunty column so we can mutate it
...
**Task** Run the following code. Describe the output in the space below. **Answer** It calculated the number of voters per 10,000 population for each county and sorted from highest to lowest to see what trends we can identify from the table.
...
**Task** Run the following code. Describe the output in the space below. **Answer**A map of voters per 10k in each MD county
...
**Task** Run the following code. Describe the output in the space below. **Answer** The colors changed to make it more visually appealing to understand
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer**It helped make the borders a bit more defined 
...
**Task** Run the following code. Describe the output in the space below. **Answer**it made a new column for unaffiliated voters per 10k people
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see, especially on the ends of the scale? **Answer**The counties with the least UNAF voters are scattered. There is the section in Baltimore as well that is amongst the lowest amount
