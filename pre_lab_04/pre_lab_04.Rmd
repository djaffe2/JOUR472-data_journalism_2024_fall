...
**Task** Run the following code to group and count records in the recipient_name column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** One problem is that all the capital letters show up first in the arrange recip name. Another problem is that the . is going before the a's 
...
**Task** Run the following code to group by recipient_clean and recipient_name, then filter for cities that start with W. Name three changes the refinr function made in the first 20 results. **Answer** It capitalized W. Millar& Co. Capitalized Walgreens. Capitalized Walmart
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by city_clean and city to examine cities that start with A. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** No. I wouldn't say it makes much of a difference because it is still just capitalizing the words 
...
```{r}
dc_food <- read_csv("data/dc-food-csv.csv")
```
